    ...
    protected void gossipKey(Node node, String key) {
        ...
        switch(remoteVersioned.getVersion().compare(localVersioned.getVersion())) {
            ...
            case AFTER: {
                ...
                // If remote version came before the local version, do nothing
            ...
            case BEFORE: {
                ...
                /*
                 * If we can't establish a causal relationship between two
                 * versions, there's a conflict. Ideally we should perform
                 * sensible reconciliation, but for simplicity's sake we will
                 * just log an error.
                 */
            case CONCURRENTLY: {
